...
public class ClientController {
    ...
    public ResponseEntity<?> getUserById(@PathVariable Long id) {
        try {
            ...
            if (client == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrorResponse("Cliente no encontrado"));
            }
            return ResponseEntity.ok(client);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ErrorResponse("Ha ocurrido algún error"));
        }
    }
